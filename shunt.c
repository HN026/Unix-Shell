...
char *arithm_expand(char *orig_expr) {
  ...
  /*
   * invert the exit status for callers who use our value to test for true/false
   * exit status, which is inverted, i.e. non-zero result is true (or zero exit
   * status) and vice versa. this is what bash does with the (( expr )) compound
   * command.
   */
  free(baseexp);
  return res2;
