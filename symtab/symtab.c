...
void init_symtab(void) {
  ...
  if (!global_symtab) {
    fprintf(stderr, "fatal error: no memory for global symtol table\n");
    exit(EXIT_FAILURE);
  }
...
/*Adds the given symbol table to the stack, and assigns the newly added table as
...
/* Removes (or pops) the symbol table on top of the stack, adjusting the stack
 * pointers as needed. */
...
/* get_local_symtab() & get_global_symtab() return pointers to the local and
 * global symbol tables, respectively. */
struct symtab_s *get_local_symtab(void) { return symtab_stack.local_symtab; }

struct symtab_s *get_global_symtab(void) { return symtab_stack.global_symtab; }

/* Returns a pointer to the symbol table stack.*/
struct symtab_stack_s *get_symtab_stack(void) { return &symtab_stack; }